...
def image_preprocessing2(im, rescale, resize_factor):
    h, w, c = 3072, 3900, 3
    nh, nw = int(h//resize_factor), int(w//resize_factor)
    # print(im.shape)
...
def image_preprocessing3(im, rescale, resize_factor):
    h, w, c = 3072, 3900, 3
    nh, nw = int(h//resize_factor), int(w//resize_factor)
    # print(im.shape)
    ...
    res = cv2.resize(im, (nw, nh), interpolation=cv2.INTER_AREA)
    ##rgb->gray scale
    gray_res = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    gray_res = cv2.resize(gray_res, (nw, nh), interpolation=cv2.INTER_AREA)
    ##GaussianBlur 
    gray_res = cv2.addWeighted(gray_res, 4, cv2.GaussianBlur(gray_res, (0, 0), 10), -4, 128) ## 30, 88
    ##vessel segmentation
    _, res2 = cv2.threshold(gray_res, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)


    if rescale == True:
        res = res / 255.
    
    res2 = np.reshape(res2, (307, 390, -1))

    total = np.concatenate([res, res2], axis=-1)
    return total

def image_preprocessing4(im, rescale, resize_factor):
    h, w, c = 3072, 3900, 3
    nh, nw = int(h//resize_factor), int(w//resize_factor)
    # print(im.shape)
    ...
    ##rgb->gray scale
    gray_res = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    gray_res1 = cv2.resize(gray_res, (nw, nh), interpolation=cv2.INTER_AREA)
    ##GaussianBlur 
    gray_res2 = cv2.addWeighted(gray_res1, 4, cv2.GaussianBlur(gray_res1, (0, 0), 10), -4, 128)
    ##vessel segmentation
    _, res2 = cv2.threshold(gray_res2, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    ...
    res2 = np.reshape(res2, (307, 390, -1))
    gray_res1 = np.reshape(gray_res1, (307, 390, -1))
    gray_res2 = np.reshape(gray_res2, (307, 390, -1))
    
    total = np.concatenate([res2, gray_res1, gray_res2], axis=-1)
